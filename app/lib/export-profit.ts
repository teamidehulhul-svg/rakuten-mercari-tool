export function exportProfit(input: {
  purchase: number; saleUsd: number; buyerShippingUsd: number; rate: number; shipping: number;
  ...
  const sale = Math.round((input.saleUsd + input.buyerShippingUsd) * input.rate);
